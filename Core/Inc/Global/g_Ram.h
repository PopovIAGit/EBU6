...
typedef struct _TFactoryParam
{
	...
	Uns		Ref15;		        // С31. 121 выбор мощности 3квт или 1.5 квт
 	...
 	Uns		reserv125;		// С35. 125 Резерв
        ...
        TPrtMode        Ov;			// C110. 200 Защита от превышения напряжения
	Uns             OvLevel;    		// C111. 201 Уровень превышения напряжения
	Uns             OvTime;     		// C112. 202 Время превышения напряжения       
        TPrtMode        Uv;			// C113. 203 Защита от превышения напряжения
	Uns             UvLevel;   		// C114. 204 Уровень понижения напряжения
	Uns             UvTime;     		// C115. 205 Время превышения напряжения        
        TPrtMode        Bv;        		// C116. 206 Защита от обрыва входных фаз
	Uns             BvLevel;   		// C117. 207 Уровень обрыва входных фаз
	Uns             BvTime;    		// C118. 208 Время обрыва входных фаз         
        TPrtMode        Phl;           		// C119. 209 Защита от обрыва фаз нагрузки
	Uns             PhlLevel;      		// C120. 210 Уровень обрыва фаз
	Uns             PhlTime;       		// C121. 211 Время определения обрыва фаз          
        TPrtMode        ShC;        		// C122. 212 Защита от короткого замыкания
	Uns		ShC_Up; 		// C123. 213 Верхняя полуволна
	Uns		ShC_Down;		// C124. 214 Нижняя полуволна     
        TPrtMode        overHeat;           	// C125. 215 Защита от обрыва фаз нагрузки
	...
	Uns             driveTemperTime;       	// C133. 223 Время определения обрыва фаз           
        TPrtMode        moduleTemper;           // C134. 224 Защита от обрыва фаз нагрузки
	Uns             moduleTemperLevel;      // C135. 225 Уровень обрыва фаз
	Uns             moduleTemperTime;       // C136. 226 Время определения обрыва фаз           
