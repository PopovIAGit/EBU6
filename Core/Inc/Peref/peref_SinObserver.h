...
typedef struct {
	...
	Float *Ramp;			// Сигнал ориентации вектора напряжения (GLOBAL_Q)
	Float  RampPrev;		// Память сигнала ориентации вектора напряжения (GLOBAL_Q)
...
typedef struct {
	...
	LgInt  Signal;			// Мгновенное значения сигнала в о.е. (GLOBAL_Q)
	LgInt  Output;			// Выходное действующее значение в о.е (GLOBAL_Q)
